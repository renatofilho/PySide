...
void DynamicQMetaObject::removeMethod(QMetaMethod::MethodType mtype, uint index)
{
    ...
    for (; it != m_d->m_methods.end(); ++it) {
        if ((it->signature() == methodSig) && (it->methodType() == mtype)){
            //qDebug() << "REMOVE METHOD" << methodSig << "OBj" << m_d->m_className;
            it->clear();
            m_d->m_invalid = true;
            break;
        }
...
int DynamicQMetaObject::addProperty(const char* propertyName, PyObject* data)
{
    ...
    if (property->d->notify) {
        ...
        if (signalNotify) {
            ...
            qDebug() << "NOTIFY ID FOR" << signalNotify << notifyId << "SIZE" << m_d->m_methods.size();
        }
    }
                                                                     ...
                                                                     int flags)
{
    ...
    for (; it != methods.end(); ++it) {
        ...
        //qDebug() << "Register method:" << it->signature() << "ON INDEX" << index << "FUNC NAME ON" << (*data)[index - 1];
...
void DynamicQMetaObject::DynamicQMetaObjectPrivate::updateMetaObject(QMetaObject* metaObj)
{
    ...
    if (!data || m_invalid) {
        ...
        qDebug() << "CREATE METAOBJECT:" << m_className;
        index = createMetaData(metaObj, strings);
        data = const_cast<uint*>(metaObj->d.data);
    } else {
        qDebug() << "UPDATE METAOBJECT:" << m_className;
    ...
    //qDebug() << "ALLOC STRINGDATA:" << newSize << "DATA:" << debugStr;
    metaObj->d.stringdata = stringdata;

    //qDebug() << "SHIFT" << m_stringDataSize;
    ...
    for(int i=0; i < m_dataSize; i++)
        printf("%d, ", metaObj->d.data[i]);
    printf("};\n");
    */

    //qDebug() << "SIGNAL NAME:" << metaObj->d.stringdata + 14;
}
